@section('title', 'Edit Item | ' . config('app.name', 'Store'))
...
<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                ...
                <h2 class="font-black text-2xl text-slate-900 leading-tight">Edit item</h2>
            </div>
    ...
    @php
        $oldTagsInput = old('tags', $item->tags);
        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            @unless ($canUpdate ?? false)
                <div class="bg-amber-50 border border-amber-200 text-amber-800 px-4 py-3 rounded-xl text-sm">
                    You do not have permission to update items. Fields are read-only.
                </div>
            @endunless

            <div class="bg-white border border-slate-100 shadow-sm rounded-2xl p-6">
                <form method="POST" action="{{ route('admin.items.update', $item->id) }}" class="space-y-6" enctype="multipart/form-data" @if(!($canUpdate ?? false)) onsubmit="return false;" @endif>
                    @csrf
                    @method('PATCH')
                    ...
                    <div class="grid gap-4 sm:grid-cols-2">
                        <label class="space-y-2">
                            ...
                            <input name="name" value="{{ old('name', $item->name) }}" required @if(!($canUpdate ?? false)) disabled @endif
                        ...
                        <label class="space-y-2">
                            ...
                            <input name="slug" value="{{ old('slug', $item->slug) }}" required @if(!($canUpdate ?? false)) disabled @endif
                                class="w-full border rounded-lg px-3 py-2 text-sm" placeholder="wireless-headphones" />
                        </label>

                        <label class="sm:col-span-2 space-y-2">
                            <span class="text-sm font-semibold text-slate-800">Description</span>
                            <textarea name="description" rows="3" class="w-full border rounded-lg px-3 py-2 text-sm" placeholder="Short description" @if(!($canUpdate ?? false)) disabled @endif>{{ old('description', $item->description) }}</textarea>
                        </label>

                        <label class="space-y-2">
                            <span class="text-sm font-semibold text-slate-800">Price *</span>
                            <input type="number" step="0.01" min="0" name="price" value="{{ old('price', $item->price) }}" required @if(!($canUpdate ?? false)) disabled @endif
                        ...
                        <label class="space-y-2">
                            ...
                            <input type="number" step="0.01" min="0" name="sale_price" value="{{ old('sale_price', $item->sale_price) }}" @if(!($canUpdate ?? false)) disabled @endif
                        ...
                        <label class="space-y-2">
                            ...
                            <input name="currency" value="{{ old('currency', $item->currency) }}" maxlength="3" minlength="3" required @if(!($canUpdate ?? false)) disabled @endif
                        ...
                        <label class="space-y-2">
                            ...
                            <input type="number" min="0" name="stock" value="{{ old('stock', $item->stock) }}" required @if(!($canUpdate ?? false)) disabled @endif
                                class="w-full border rounded-lg px-3 py-2 text-sm" placeholder="25" />
                        </label>

                        <label class="sm:col-span-2 space-y-2">
                            <span class="text-sm font-semibold text-slate-800">Tags</span>
                            <input name="tags" value="{{ $oldTagsString }}" class="w-full border rounded-lg px-3 py-2 text-sm" @if(!($canUpdate ?? false)) disabled @endif
                        ...
                        <label class="sm:col-span-2 space-y-2">
                            ...
                            <input type="url" name="image_url" value="{{ old('image_url', $item->image_url) }}" @if(!($canUpdate ?? false)) disabled @endif
                        ...
                        <label class="sm:col-span-2 space-y-2">
                            ...
                            <input type="file" name="image" accept="image/*" @if(!($canUpdate ?? false)) disabled @endif
                                class="w-full border rounded-lg px-3 py-2 text-sm" />
                            <p class="text-xs text-slate-500">If provided, uploaded image will replace the URL.</p>
                        </label>
                    </div>
                    ...
                    <div class="flex justify-between items-center">
                        ...
                        <div class="flex gap-3">
                            @if ($canDelete ?? false)
                                <a href="{{ route('admin.items.delete', $item->id) }}" class="text-sm text-rose-600">Delete</a>
                            @endif
                            <button type="submit"
